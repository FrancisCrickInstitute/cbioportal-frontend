...
export default class PatientCCFColumnFormatter {
    ...
    public static getComponentForSampleArgs<T extends {mutCCF:number,molecularProfileId:string}>(mutation:T) {
    ...
    public static convertMutationToSampleElement<T extends {sampleId:string, mutCCF:number, molecularProfileId:string}>(mutation:T, color:string, barX:number, sampleComponent:any) {
            const ccf = mutation.mutCCF;
            if (ccf < 0) {
                return null;
            }
    ...
    public static renderFunction(mutations:Mutation[], sampleManager:SampleManager|null) {
        ...
        const barX = sampleOrder.reduce((map, sampleId:string, i:number) => {map[sampleId] = PatientCCFColumnFormatter.indexToBarLeft(i); return map;}, {} as {[s:string]:number});
